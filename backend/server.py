...
GROQ_BASE_URL = os.environ.get("GROQ_BASE_URL") or os.environ.get("GROK_BASE_URL") or "https://api.groq.com/openai/v1"
GROQ_BASE_URL = GROQ_BASE_URL.rstrip("/")
GROQ_CHAT_MODEL = os.environ.get("GROQ_CHAT_MODEL") or os.environ.get("GROK_CHAT_MODEL") or "llama-3.3-70b-versatile"
...
LLM_PROVIDER = "groq" if LLM_PROVIDER == "grok" else LLM_PROVIDER
LLM_PROVIDERS = ["groq" if p == "grok" else p for p in LLM_PROVIDERS]
LLM_EXAM_PROVIDER = "groq" if LLM_EXAM_PROVIDER == "grok" else LLM_EXAM_PROVIDER
...
def _resolve_llm_providers() -> List[str]:
    if LLM_PROVIDER in {"openai", "groq", "gemini", "nvidia"}:
        return [LLM_PROVIDER]
    if LLM_PROVIDER == "hybrid":
        configured = LLM_PROVIDERS or ["gemini", "nvidia"]
        deduped: List[str] = []
        for provider in configured:
            if provider in {"openai", "groq", "gemini", "nvidia"} and provider not in deduped:
...
async def _generate_with_llm_internal(
    ...
    if not providers:
        raise HTTPException(
            status_code=500,
            detail="LLM_PROVIDER is invalid. Use openai, groq, gemini, nvidia, or hybrid.",
        )
    ordered_providers = await _provider_order_for_request(providers)
    if (generation_type or "").strip().lower() == "exam" and LLM_EXAM_PROVIDER in {"openai", "groq", "gemini", "nvidia"}:
...
async def repair_json_with_llm(raw_text: str, generation_type: str) -> Dict[str, Any]:
    ...
    # Prefer Gemini first when available, then Groq/OpenAI, and only then NVIDIA.
...
async def startup_checks():
    ...
    if LLM_PROVIDER not in {"openai", "groq", "gemini", "nvidia", "hybrid"}:
        raise RuntimeError("LLM_PROVIDER must be 'openai', 'groq', 'gemini', 'nvidia', or 'hybrid'")
    ...
    if LLM_EXAM_PROVIDER and LLM_EXAM_PROVIDER not in {"openai", "groq", "gemini", "nvidia"}:
        raise RuntimeError("LLM_EXAM_PROVIDER must be one of: openai, groq, gemini, nvidia (or empty)")
